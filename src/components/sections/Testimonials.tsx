...
export function Testimonials() {
  ...
  return (
    <section id="omtaler" className="py-20 sm:py-24 bg-[#edf5ed]">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          ...
          {featured.map((t, i) => (
            <AnimateOnScroll key={t.name} delay={(i % 3) as 0|1|2|3|4}>
              <div className={`relative flex flex-col rounded-xl border p-6 h-full ${
                ...
                <div className="mt-4 pt-4 border-t border-[#cce0cc]">
                  <p className="font-semibold text-[#1a2b1a] text-sm">{t.name}</p>
                  <p className="text-xs text-[#5a6e5a]">{t.job} · {t.year}</p>
                </div>
              </div>
            </AnimateOnScroll>
          ))}
        ...
        {!showAll && (
          <AnimateOnScroll className="text-center mb-6">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-[#cce0cc] px-5 py-2.5 text-sm font-medium text-[#5a6e5a] hover:border-[#1e6ec8]/40 hover:text-[#1a2b1a] hover:bg-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#1e6ec8]"
            >
              Vis alle 7 omtaler ↓
            </button>
          </AnimateOnScroll>
        ...
        {showAll && (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 mt-2">
            {extra.map((t, i) => (
              <AnimateOnScroll key={t.name} delay={(i % 3) as 0|1|2|3|4}>
                <div className="rounded-xl border border-[#cce0cc] bg-white p-5">
                  <Stars />
                  <blockquote className="mt-3 text-sm text-[#5a6e5a] leading-relaxed italic">
                    "{t.text}"
                  </blockquote>
                  <div className="mt-3 pt-3 border-t border-[#cce0cc]">
                    <p className="font-semibold text-[#1a2b1a] text-sm">{t.name}</p>
                    <p className="text-xs text-[#5a6e5a]">{t.job} · {t.year}</p>
                  </div>
                </div>
