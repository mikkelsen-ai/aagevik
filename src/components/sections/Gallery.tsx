...
export function Gallery() {
  return (
    <section id="galleri" className="py-20 sm:py-24 bg-[#0d2847]">
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <AnimateOnScroll>
          <div className="mb-12 text-center">
            ...
            <h2 className="font-display text-3xl font-bold text-[#f2efe9] sm:text-4xl">
              Ferdig utførte arbeider
            </h2>
            <p className="mt-3 text-[#8fa98f] max-w-xl mx-auto">
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-3">
          {images.map((image, i) => (
            <AnimateOnScroll key={image.src} delay={(i % 3) as 0|1|2|3|4}>
              <div className="group relative aspect-square overflow-hidden rounded-xl border border-[#1e3a5f]">
                <Image
                  src={image.src}
                  alt={image.alt}
                  fill
                ...
                />
                {/* Hover overlay */}
                <div className="absolute inset-0 bg-gradient-to-t from-[#1c3d1d]/80 via-[#1c3d1d]/20 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
                <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-2 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
                  <p className="text-sm font-medium text-[#f2efe9]">Prosjekt av Åge Vik</p>
                  <p className="text-xs text-[#8fa98f]">Bergen</p>
                </div>
              </div>
            </AnimateOnScroll>
          ))}
