...
export function TrustBar() {
  return (
    <section className="border-y border-[#cce0cc] bg-[#edf5ed] py-8">
      <div className="mx-auto max-w-5xl px-4 sm:px-6">
        <div className="grid grid-cols-2 gap-6 sm:grid-cols-4">
          {signals.map((signal, i) => (
            <AnimateOnScroll key={signal.text} delay={i as 0|1|2|3|4}>
              <div className="flex flex-col items-center gap-2 text-center">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#1e6ec8]/10 text-[#1e6ec8]">
                  <signal.icon className="h-5 w-5" />
                </div>
                <div>
                  <p className="font-sans text-lg font-bold tracking-tight text-[#1a2b1a]">{signal.text}</p>
                  <p className="text-xs text-[#5a6e5a]">{signal.sub}</p>
                </div>
              </div>
            </AnimateOnScroll>
          ))}
